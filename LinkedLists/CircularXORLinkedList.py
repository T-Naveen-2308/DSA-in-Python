...
class CircularXorLinkedList:
    ...
    def insert_begin(self, data):
        ...
        self.ll.append(temp)
        if self.head:
            temp.pn = id(self.head)^id(self.tail)
            self.head.pn = (self.head.pn^id(self.tail))^id(temp)
            self.head = temp
        else:
            self.head = temp
            self.tail = temp
    ...
    def insert_end(self, data):
        ...
        self.ll.append(temp)
        if self.head:
            temp.pn = id(self.tail)^id(self.head)
            self.tail.pn = (self.tail.pn^id(self.head))^id(temp)
            self.tail = temp
        else:
            self.head = temp
            self.tail = temp
    ...
    def delete_begin(self):
        if self.head==0:
            ...
            print('Deleted', self.head.data)
            temp = self.re_by_add(self.head.pn^id(self.tail))
            temp.pn = (temp.pn^id(self.head))^id(self.tail)
            self.tail.pn = (self.tail.pn^id(self.head))^id(temp)
            self.head = temp
            self.count -= 1
    ...
    def delete_end(self):
        if self.head==0:
            ...
            temp.pn = (temp.pn^id(self.tail))^id(self.head)
            self.head.pn = (self.head.pn^id(self.tail))^id(temp)
            self.tail = temp
            self.count -= 1
    ...
    def display_forward(self):
        if self.head==0:
            ...
            temp = self.head
            next_node = self.re_by_add(self.head.pn^id(self.tail))
            p = id(self.head)
            t = 0
            while next_node!=self.head:
                ...
                t = id(next_node)
                temp = next_node
                next_node = self.re_by_add(next_node.pn^p)
                p = t
            print(temp.data)
    ...
    def display_backward(self):
        if self.head==None:
            ...
            print('The elements are:')
            temp = self.tail
            next_node = self.re_by_add(self.tail.pn^id(self.head))
            p = id(self.tail)
            t = 0
            while next_node!=self.tail:
                ...
                t = id(next_node)
                temp = next_node
                temp = self.re_by_add(next_node.pn^p)
                p = t
            print(temp.data)
...
linked_list = CircularXorLinkedList()
